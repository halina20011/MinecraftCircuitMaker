...
GLuint loadAllBlocks(const char textureFile[]){
    ...
    uint8_t w, h;
    memcpy(&w, data, sizeof(uint8_t));
    memcpy(&h, data + sizeof(uint8_t), sizeof(uint8_t));
    
    printf("texture %ux%u\n", w, h);

    return loadTexture(data + sizeof(uint8_t) * 2, w, h);
    // return loadTexture(data, 64, 64);
}
    ...
    if(d){
        while((dir = readdir(d)) != NULL){
            if(dir->d_type != DT_DIR){
                ...
                printf("%s %i\n", filePath, dir->d_type);
                struct BlockType blockType = readBlock(filePath);
